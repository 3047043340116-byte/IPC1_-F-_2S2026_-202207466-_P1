...
public class EspacioRefugioDatos {
    ...
    public int contarOcupadosEnFila(int fila) {
        ...
        for (int j = 0; j < columnas; j++) {
            if (!espacios[fila][j].estaDisponible()) {
                ocupados++;
            }
        }
    ...
    public int contarDisponiblesEnFila(int fila) {

        if (fila < 0 || fila >= filas) {
            return -1;
        }

        return columnas - contarOcupadosEnFila(fila);
    }
